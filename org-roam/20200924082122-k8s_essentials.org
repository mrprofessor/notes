...
- With a orchestration tool many manual operations like deploying code changes to various containers, scaling up the service by adding new containers etc can be automated. Kubernetes and docker swarm are some reknowend orchestration tools out there.
